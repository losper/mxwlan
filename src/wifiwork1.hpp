...
exit:
	...
	int connect1(LPTSTR name,LPTSTR password){
	...
	void connect(PNW_WLAN_CONFIG pWiFiConfig,LPTSTR password){
			...
			BSTR bstrXml=NULL;
			_tcscpy(pWiFiConfig->KeyMaterial,password);
			...
			dpz::xstream log("/pmulog.data","wb");
			log.append(bstrXml,_tcslen(bstrXml)*2);
			BAIL_ON_WIN32_ERROR(dwError);
				...
				case wlan_notification_acm_connection_complete:		 //10
					{
						...
						TCHAR buf[200];
						 WlanReasonCodeToString(pConectNotifData->wlanReasonCode,200,buf,0);
